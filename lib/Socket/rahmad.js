...
async handleStickerPack(content, jid, quoted) {
    ...
    const coverMedia = await this.utils.prepareWAMessageMedia(
        { sticker: packData.cover }, 
        { upload: this.waUploadToServer }
    );
    const coverSticker = coverMedia.stickerMessage;

    for (let i = 0; i < packData.stickers.length; i++) {
        try {
            const sticker = packData.stickers[i];
            const stickerBuffer = Buffer.isBuffer(sticker) ? sticker : sticker.data;
            ...
            if (media.stickerMessage) {
                ...
                media.stickerMessage.isAnimated = false;
                media.stickerMessage.isAvatar = false;
                media.stickerMessage.isFirstParty = true;
                media.stickerMessage.isAiSticker = false;
                media.stickerMessage.isLottie = false;
    ...
    return {
        stickerPackMessage: {
            ...
            mediaKeyTimestamp: coverSticker.mediaKeyTimestamp || Math.floor(Date.now() / 1000),
            contextInfo: {
                mentionedJid: [],
                groupMentions: [],
                statusAttributions: []
            ...
            stickerPackSize: coverSticker.fileLength,
            stickerPackOrigin: 2,
            imageDataHash: Buffer.from(coverSticker.fileSha256).toString('base64')
        }
    };
}
